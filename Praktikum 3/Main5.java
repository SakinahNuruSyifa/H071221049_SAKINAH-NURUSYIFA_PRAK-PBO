...
public class Main5 {
    ...
    public static void main(String[] args) {
        ...
        User user = new User("Syifa", 5_000); // add User
        ...
        produk.add(new Product("Samsung Galaxy A23", "2k", 1));
        produk.add(new Product("Samsung Galaxy S22", "4k", 8));
        produk.add(new Product("IPhone 14 pro", 1500 , 10));

        Store sisfoKomputer = new Store("Sisfo Komputer", produk); // Instance class Store
        sisfoKomputer.addProduct(new Product("Mac", "10K", 12));

        while (true) { // Perulangan Pertama 

            user.card(); // untuk Menampilkan data User
            sisfoKomputer.listProduct(); // untuk Menampilkan list Product yg akan ditawarkan
            System.out.print("> ");
            ...
            if (pilihan1 > sisfoKomputer.getProduct().size()) { // Opsi yang diinput melebihi jumlah produk
                ...
                System.out.println("-".repeat(45)); // garis
